...
class StateMEC(NamedTuple):
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        return StateMEC(
            ...
            selected_=(
                ...
                else torch.ones(batch_size, 1, (n_node+1 + 63 ) // 64, dtype=torch.int64, device=loc.device)  # Ceil
            ),
            prec=torch.zeros(batch_size, n_node+1, 1, device=loc.device),
            succ=torch.zeros(batch_size, n_node+1, 1, device=loc.device),
            prev_a = torch.ones(batch_size, 1, dtype=torch.long, device=loc.device)*(-1),  # -1 is to dopet
            visited_=(  # Visited as mask is easier to understand, as long more memory efficient
                ...
                if visited_dtype == torch.uint8
                else torch.zeros(batch_size, 1, (n_node+1 + 63) // 64, dtype=torch.int64, device=loc.device)  # Ceil
            ),
            free=torch.ones(batch_size, n_node+1, 1, device=loc.device),
    ...
    def initdep(self, dependency):
        ...
        if len(dependency) > 1 :
            for i in range(1,len(dependency)):
                pre[dependency[i]+1]=dependency[i-1]+1
            for i in range(len(dependency)-1):
                suc[dependency[i]+1]=dependency[i+1]+1
    ...
    def get_mask(self):
        ...
        mask = ((self.selected < 1) | (self.visited > 0) | (self.free < 0).transpose(-1,-2))| (left + cost_time > tw_right).transpose(-1,-2) 
        # 通常情况下 , 0 即初始位置被mask了
        all_true = mask.squeeze(1).all(dim=-1)
        # 对于全为 True 的 batch，将第 0 个位置的元素设置为 False
        mask[all_true, 0, 0] = False
        return mask
